...
export default function ClientMenuPage({
    ...
    const { tc, tt, td } = useTranslateContent();
    ...
    // Table State
    const [currentTable, setCurrentTable] = useState<string | null>(tableNumber || tableParam || tableId);
    ...
    const formatPrice = (price: number) => {
        try {
            return new Intl.NumberFormat(initialLocale || 'en', {
                style: 'currency',
                currency: currency,
    ...
    useEffect(() => {
        if (paymentSuccess === 'true') {
            toast.success(t('paymentSuccessMsg'), { duration: 5000, icon: '🎉' });
            // Clear URL param
    ...
    const handleCompleteOrder = async () => {
        ...
        try {
            if (restaurant.is_lite_mode) {
                setCart([]);
                setShowCartModal(false);
                if (isClient) localStorage.removeItem(`cart_${restaurant.id}`);
                toast.success(t('liteModeOrderMsg'), { duration: 6000, icon: '💡' });
                setIsOrdering(false);
                return;
            }
            ...
            if (result.success) {
                ...
                toast.error(t(result.error || 'orderError'));
            }
        } catch (error) {
            console.error(error);
    ...
    const addToCart = (product: Product) => {
        ...
        if (!existing) {
            ...
            toast.success(`${product.name} ${t('qtyIncreased')}`, { icon: '➕' });
        }
    ...
    return (
        <div className="light min-h-screen bg-white pb-28 font-sans antialiased text-gray-900 transition-colors duration-300">

            {/* TEST QR CODE FOR DEMOREST DIRECT DEMO (DESKTOP) */}
            {restaurant.slug === 'demorest' && (
                ...
                {showCampaign && (
                    ...
                    >
                        <motion.div
                            ...
                            className="bg-white dark:bg-gray-900 w-full max-w-sm rounded-2xl shadow-2xl p-6 relative overflow-hidden transition-colors"
                        >
                            ...
                            <button
                                ...
                                className="absolute top-4 right-4 text-gray-400 hover:text-gray-900 dark:hover:text-white bg-gray-100 dark:bg-gray-800 rounded-full p-1 z-10 transition-colors"
                            >
                                <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg>
                            </button>

                            <div className="text-center pt-2 relative z-0">
                                <div className="inline-flex items-center justify-center w-16 h-16 bg-purple-100 dark:bg-purple-900/30 text-purple-600 dark:text-purple-400 rounded-full mb-4 text-3xl shadow-sm animate-bounce">
                                    🎉
                                </div>
                                <h3 className="text-xl font-bold text-gray-900 dark:text-white mb-2">
                                    {restaurant.campaign_title || t('specialOffer')}
                                </h3>
                                ...
                                <button
                                    onClick={() => setShowCampaign(false)}
                                    className="w-full py-3 bg-black dark:bg-white text-white dark:text-black font-medium rounded-xl shadow-lg hover:bg-gray-800 dark:hover:bg-gray-200 transition-all active:scale-95"
                                >
                                    {t('awesome')}
                                </button>
                            </div>
            ...
            {selectedProduct && (
                <ProductDetailModal
                    ...
                    isLiteMode={restaurant.is_lite_mode}
                />
            )}
                ...
                {showCartModal && (
                    ...
                    >
                        <motion.div
                            ...
                            className="bg-white dark:bg-gray-900 w-full max-w-md sm:rounded-2xl rounded-t-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh] transition-colors"
                        >
                            <div className="p-4 border-b dark:border-gray-800 flex items-center justify-between bg-gray-50 dark:bg-gray-800/50">
                                <h3 className="font-bold text-lg text-gray-900 dark:text-white">{t('order')} ({cartCount})</h3>
                                <button onClick={() => setShowCartModal(false)} className="p-2 bg-gray-200 dark:bg-gray-700 rounded-full hover:bg-gray-300 dark:hover:bg-gray-600 transition-colors">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg>
                                </button>
                            </div>
                            ...
                            <div className="flex-1 overflow-y-auto p-4 space-y-4">
                                {cart.length === 0 ? (
                                    ...
                                    cart.map((item) => (
                                        <div key={item.product.id} className="flex items-center gap-4 bg-gray-50 dark:bg-gray-800/40 p-3 rounded-xl border border-gray-100 dark:border-gray-800">
                                            <div className="w-16 h-16 bg-white dark:bg-gray-800 rounded-lg overflow-hidden border border-gray-200 dark:border-gray-700 flex-shrink-0">
                                                {item.product.image_url ? (
                                                    ...
                                                    <div className="w-full h-full flex items-center justify-center text-xs text-gray-300 dark:text-gray-500">IMG</div>
                                                )}
                                            </div>
                                            <div className="flex-1">
                                                <h4 className="font-bold text-sm text-gray-900 dark:text-white line-clamp-1">{item.product.name}</h4>
                                                <div className="text-sm font-semibold text-gray-600 dark:text-gray-400">{formatPrice(item.product.price * item.quantity)}</div>
                                            </div>
                                            <div className="flex items-center gap-3 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg px-2 py-1 shadow-sm">
                                                <button onClick={() => updateQuantity(item.product.id, -1)} className="w-6 h-6 flex items-center justify-center text-gray-500 dark:text-gray-400 hover:text-black dark:hover:text-white font-bold transition-colors">-</button>
                            ...
                            <div className="p-4 border-t dark:border-gray-800 bg-gray-50 dark:bg-gray-800/80 space-y-3 transition-colors">
                                <div className="flex justify-between items-center mb-2 text-lg font-bold text-gray-900 dark:text-white">
                                    <span>{t('total')}</span>
                                    <span>{formatPrice(cartTotal)}</span>
                                </div>

                                {restaurant.is_payment_enabled && !restaurant.is_lite_mode && (
                                    <button
                                ...
                                <button
                                    ...
                                    disabled={cart.length === 0 || isOrdering || isPaying}
                                    className={`w-full py-4 font-bold rounded-xl shadow-lg disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 transition-all active:scale-[0.98] ${restaurant.is_payment_enabled ? 'bg-white dark:bg-gray-900 border-2 border-black dark:border-white text-black dark:text-white hover:bg-gray-100 dark:hover:bg-gray-800' : 'bg-black dark:bg-white text-white dark:text-black hover:bg-gray-800 dark:hover:bg-gray-200'}`}
                                >
                                    {isOrdering ? (
            ...
            <div className="relative h-72 md:h-96 bg-gray-900 transition-all duration-500 overflow-hidden">
                ...
                {/* Arkaplan Görseli */}
                <img
                    src={restaurant.cover_image_url || "/images/cover-placeholder.svg"}
                    className="w-full h-full object-cover opacity-60 scale-105"
                    alt="Cover"
                    onError={(e) => {
                        const t = e.target as HTMLImageElement;
                        if (!t.src.includes('cover-placeholder')) t.src = '/images/cover-placeholder.svg';
                    }}
                />
                ...
                <div className="absolute bottom-6 left-6 right-6 z-20 text-white md:bottom-10 md:left-10 max-w-5xl mx-auto flex flex-col md:flex-row md:items-end justify-between gap-6">
                    ...
                    <div className="flex-1 flex flex-col gap-3">
                        {/* Masa Bilgisi */}
                        {currentTable && !restaurant.is_lite_mode && (
                            <div className="self-start inline-flex items-center gap-2 bg-orange-500 text-white px-3 py-1 rounded-full text-xs font-bold shadow-lg animate-pulse mb-1">
                                <span>🪑 Table Selected ({currentTable})</span>
                            </div>
                        )}
                            ...
                            <div className="flex flex-col flex-1 justify-center">
                                ...
                                <div className="flex flex-wrap items-center gap-2 mt-1">
                                    ...
                                    <span className="flex items-center gap-1.5 text-xs md:text-sm text-gray-200 drop-shadow-md font-medium">
                                        ...
                                        {restaurant.address || t('addressNotSpecified')}
                                    </span>
                                </div>
                            </div>
                        </div>
                    ...
                    <div className="shrink-0 mt-6 md:mt-0 flex flex-row items-center justify-between w-full md:w-auto gap-4">
                        ...
                        <div className="relative">
                            <select
                                ...
                                className="appearance-none bg-black/60 dark:bg-white/10 text-white backdrop-blur-md px-4 py-2.5 md:py-3 rounded-full text-sm font-semibold shadow-xl cursor-pointer pr-8 border border-white/10 dark:border-white/5 outline-none hover:bg-black/80 dark:hover:bg-white/20 transition-colors"
                            >
                        ...
                        <button
                            ...
                            className="bg-white dark:bg-gray-800 text-black dark:text-white px-5 py-2.5 md:py-3 rounded-full text-sm font-bold shadow-xl active:scale-95 transition-all flex items-center justify-center gap-2 hover:bg-gray-100 dark:hover:bg-gray-700"
                        >
            ...
            {/* CATEGORIES / LIST */}
            <div className="sticky top-0 z-30 bg-white/95 dark:bg-gray-900/95 backdrop-blur-md shadow-sm border-b border-gray-100 dark:border-gray-800 py-4 overflow-x-auto no-scrollbar transition-colors">
                <div className="flex px-4 gap-5 md:justify-center">
                    {categories?.map((cat) => (
                        ...
                        >
                            <div className="w-16 h-16 rounded-full bg-gray-50 border-2 border-gray-100 group-hover:border-black transition-all overflow-hidden relative shadow-sm group-hover:scale-110">
                                <img 
                                    src={cat.products?.[0]?.image_url || '/images/food-placeholder.svg'} 
                                    alt={tc(cat.name)} 
                                    className="w-full h-full object-cover"
                                    onError={(e) => {
                                        const t = e.target as HTMLImageElement;
                                        if (!t.src.includes('food-placeholder')) t.src = '/images/food-placeholder.svg';
                                    }}
                                />
                            </div>
                            <span className="text-[11px] font-bold text-gray-500 group-hover:text-black line-clamp-1 text-center transition-colors">
                                {tc(cat.name)}
                            </span>
                        </a>
                    ))}
                ...
                {categories?.filter((c) => c.products?.length > 0).map((category) => (
                    <section key={category.id} id={`cat-${category.id}`} className="scroll-mt-40">
                        <div className="flex items-center gap-3 mb-5">
                            <h2 className="text-xl md:text-2xl font-bold text-gray-800 dark:text-white transition-colors">{tc(category.name)}</h2>
                            <div className="h-0.5 bg-gray-100 dark:bg-gray-800 flex-1 transition-colors"></div>
                        </div>
                            ...
                            {category.products?.map((product) => {
                                ...
                                return (
                                    <motion.div
                                        ...
                                        className={`bg-white dark:bg-gray-900 rounded-2xl p-3 shadow-sm border border-gray-100 dark:border-gray-800 flex gap-4 min-h-[120px] transition-all hover:shadow-md cursor-pointer ${!product.is_available ? 'opacity-60 grayscale' : ''}`}
                                    >
                                        <div className="flex-1 flex flex-col justify-between py-1">
                                            <div>
                                                <h3 className="font-bold text-gray-900 dark:text-white leading-tight mb-1 text-base transition-colors line-clamp-2 h-10">{product.name}</h3>
                                                <p className="text-xs text-gray-500 dark:text-gray-400 line-clamp-2 leading-relaxed transition-colors">{td(product)}</p>

                                                {/* PRODUCT TAG BADGES - Zero Language Leak */}
                                                {product.tags && product.tags.length > 0 && (
                                                    <div className="flex flex-wrap gap-1 mt-1.5 mb-0.5">
                                                        {product.tags.map(tag => {
                                                            const config = PRODUCT_TAGS[tag];
                                                            if (config) {
                                                                // Known tag: use icon + translated label
                                                                return (
                                                                    <span key={tag} className={`inline-flex items-center gap-0.5 px-1.5 py-0.5 rounded-md text-[10px] font-bold shadow-sm ${config.colorClass}`}>
                                                                        <span>{config.icon}</span>
                                                                        <span>{tt(config.translationKey)}</span>
                                                                    </span>
                                                                );
                                                            }
                                                            // Unknown tag: try tt(tag) to look it up in client.tags dict
                                                            // tt() returns the key unchanged if not found — acceptable generic fallback
                                                            return (
                                                                <span key={tag} className="inline-flex items-center px-1.5 py-0.5 rounded-md text-[10px] font-bold bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-300 border border-gray-200 dark:border-gray-700 shadow-sm">
                                                                    {tt(tag)}
                                                                </span>
                                                            );
                                                        })}
                                                ...
                                                {!product.is_available ? (
                                                    <span className="inline-block mt-2 text-[10px] font-bold bg-red-100 dark:bg-red-900/30 text-red-600 dark:text-red-400 px-2 py-1 rounded">
                                                        {t('soldOut')}
                                                    </span>
                                                ) : (
                                                    <div className="flex items-center gap-2 mt-2">
                                                        {restaurant.show_calories !== false && product.calories > 0 && (
                                                            <span className="text-[10px] bg-orange-50 dark:bg-orange-900/20 text-orange-600 dark:text-orange-400 px-1.5 py-0.5 rounded font-medium flex items-center gap-0.5 transition-colors">
                                                                🔥 {product.calories} {t('kcal')}
                                                            </span>
                                                        )}
                                                        {restaurant.show_preparation_time !== false && product.preparation_time > 0 && (
                                                            <span className="text-[10px] bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400 px-1.5 py-0.5 rounded font-medium flex items-center gap-0.5 transition-colors">
                                            ...
                                            <div className="mt-2 flex items-center justify-between">
                                                <span className="text-lg font-bold text-gray-900 dark:text-white transition-colors">{formatPrice(product.price)}</span>
                                                {product.is_available && !restaurant.is_lite_mode && (
                                                    <div className="flex items-center gap-2">
                                                        {product.video_url && (
                                                            <a
                                                                ...
                                                                className="w-8 h-8 rounded-full bg-red-50 dark:bg-red-900/30 flex items-center justify-center text-red-600 dark:text-red-400 animate-pulse hover:bg-red-200 transition-colors"
                                                            >
                                                        ...
                                                        <button
                                                            ...
                                                            className={`px-4 py-1.5 rounded-full text-[10px] font-bold transition-all shadow-sm active:scale-95 ${inCart ? 'bg-green-600 text-white' : 'bg-black dark:bg-white text-white dark:text-black hover:bg-gray-800 dark:hover:bg-gray-200'}`}
                                                        >
                                        ...
                                        <div className="w-28 h-28 md:w-32 md:h-32 bg-gray-50 dark:bg-zinc-900 rounded-xl overflow-hidden flex-shrink-0 relative shadow-[inset_0_0_10px_rgba(0,0,0,0.05)] self-center transition-all border border-gray-100 dark:border-zinc-800">
                                            {product.image_url ? (
                                                <img 
                                                    ...
                                                    onError={(e) => {
                                                        ...
                                                        t.parentElement!.innerHTML = `<div class="w-full h-full flex items-center justify-center bg-gray-100 dark:bg-zinc-900"><svg xmlns='http://www.w3.org/2000/svg' className='w-10 h-10 text-gray-300' fill='none' viewBox='0 0 24 24' stroke='currentColor'><path stroke-linecap='round' stroke-linejoin='round' stroke-width='1' d='M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17m0 0a2 2 0 100 4 2 2 0 000-4zm-8 2a2 2 0 11-4 0 2 2 0 014 0z'/></svg></div>`;
                                                    }}
                                                />
                                            ) : (
                                                <div className="w-full h-full flex items-center justify-center bg-gray-100 dark:bg-zinc-900">
                                                    <svg xmlns="http://www.w3.org/2000/svg" className="w-10 h-10 text-gray-300 dark:text-zinc-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            {/* BOTTOM NAV BAR */}
            <div className="fixed bottom-0 left-0 right-0 bg-white/95 dark:bg-gray-900/95 backdrop-blur-xl border-t border-gray-100 dark:border-gray-800 py-2 pb-6 z-40 shadow-[0_-8px_30px_-5px_rgba(0,0,0,0.08)] transition-colors">
                <div className="flex justify-between items-center max-w-md mx-auto relative px-4">
                    {/* Menü Butonu */}
                    <button onClick={() => window.scrollTo(0, 0)} className="flex flex-col items-center gap-1 text-black dark:text-white p-2 flex-1 transition-colors">
                    ...
                    {/* Merkez Sepet Butonu - Hidden in Lite Mode */}
                    {!restaurant.is_lite_mode && (
                        <div className="flex-1 flex justify-center mt-[-24px]">
                            <button
                                onClick={() => setShowCartModal(true)}
                                className={`relative flex items-center justify-center w-14 h-14 rounded-full shadow-2xl transition-all duration-300 hover:scale-105 active:scale-95 ${cartCount > 0 ? 'bg-black dark:bg-white text-white dark:text-black' : 'bg-gray-100 dark:bg-gray-800 text-gray-500 dark:text-gray-400'}`}
                            >
                                <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z" />
                                </svg>
                                {cartCount > 0 && (
                                    <span className="absolute -top-1 -right-1 bg-red-600 text-white text-[10px] font-extrabold w-5 h-5 flex items-center justify-center rounded-full border-2 border-white dark:border-gray-950 shadow-md animate-pulse">
                                        {cartCount}
                                    </span>
                                )}
                            </button>
                    ...
                    {!restaurant.is_lite_mode && (
                        <button
                            ...
                            className="flex flex-col items-center gap-1 text-gray-500 dark:text-gray-400 hover:text-black dark:hover:text-white transition-colors p-2 flex-1"
                        >
                ...
                {orderSuccess && (
                    ...
                    >
                        <motion.div
                            initial={{ scale: 0.8 }}
                            animate={{ scale: 1 }}
                            className="bg-white dark:bg-gray-900 w-full max-w-sm rounded-3xl p-8 text-center transition-colors shadow-2xl"
                        >
                            <div className="w-20 h-20 bg-green-100 dark:bg-green-900/30 text-green-600 dark:text-green-400 rounded-full flex items-center justify-center mx-auto mb-6 text-4xl shadow-sm">
                                🚀
                            </div>
                            <h3 className="text-2xl font-bold text-gray-900 dark:text-white mb-2">{t('orderSuccess')}</h3>
                            <p className="text-gray-500 dark:text-gray-400 mb-8">{t('orderSuccessMsg')}</p>

                            <div className="bg-gray-50 dark:bg-gray-800/50 rounded-xl p-4 mb-6 border border-gray-100 dark:border-gray-800">
                                <p className="font-medium text-sm text-gray-800 dark:text-gray-200">Order Tracking</p>
                                ...
                                <p className="text-xs text-gray-400 dark:text-gray-500 mt-2 text-left">Preparing...</p>
                            </div>

                            <button
                                onClick={() => setOrderSuccess(false)}
                                className="w-full py-3 bg-gray-900 dark:bg-white text-white dark:text-black font-bold rounded-xl hover:bg-black dark:hover:bg-gray-100 transition-colors"
                            >
                                OK
                            </button>
                        </motion.div>
